package com.attinad.automation.common;

/**
 * Created by whiterabbit (karthik@attinadsoftware.com) on 20/01/17.
 */
public class Constants {
    public static final String TARGET_OS_PROPERTY = "cantiz.automation.environment.target-os";
    public static final String TARGET_BROWSER_PROPERTY = "cantiz.automation.environment.target-browser";
    public static final String WEB_DRIVER_TYPE_PROPERTY = "cantiz.automation.environment.web-driver-type";
    public static final String WEB_DRIVER_URL_PROPERTY = "cantiz.automation.environment.web-driver-url";
    public static final String TARGET_URL_PROPERTY = "cantiz.automation.environment.target-url";
    public static final String TARGET_ELECTRONURL_PROPERTY = "cantiz.automation.environment.target-electronurl";
    public static final String ELECTRON_TOKEN_PROPERTY = "cantiz.automation.environment.token";
    public static final String SUPERUSER_TOKEN_PROPERTY = "cantiz.automation.environment.superUserToken";
    

    public static final String MAIL_HOST_PROPERTY = "cantiz.automation.mailer.host";
    public static final String MAIL_PORT_PROPERTY = "cantiz.automation.mailer.port";
    public static final String MAIL_STORE_TYPE_PROPERTY = "cantiz.automation.mailer.store-type";
    public static final String MAIL_USERNAME_PROPERTY = "cantiz.automation.mailer.username";
    public static final String MAIL_PSWD_PROPERTY = "cantiz.automation.mailer.password";
    
    public static final String TEXTBOX = "textbox";
    public static final String DIVELEMENT = "divelement";
    public static final String SPANELEMENT = "spanelement";
}
